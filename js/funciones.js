...
function crearTabla(lista, texto) {
  ...
  titulo +=
    '<h1 id="tituloTabla" class="m-0 mb-2 ' +
    config.color +
    '">' +
    texto +
    "</h1>";
  ...
  window.tabla = new Tabulator("#tablas", {
    ...
    placeholder:
      '<div class="py-4 text-center"> <i class="bi ' +
      config.icono +
      ' fs-2 d-block mb-2 text-dark"></i> <strong class="text-black">No hay registros</strong> </div>',
    pagination: true,
    ...
    headerElement:
      '<div class="text-center py-3 bg-primary text-white"><h2 class="m-0">' +
      texto +
      "</h2></div>",
...
function celdaNombre(cell) {
  ...
  if (datos[config.pendiente] == 1) {
    let codigo =
      '<div class="position-relative"><div class="text-center"><span class="d-inline-block w-75">(Pendiente)<br>' +
      datos[config.nombre] +
      "</span></div>";

    codigo +=
      '<div class="tresPuntos position-absolute top-50 end-0 me-2"><div class="botPuntos btn p-0 border-0" data-bs-toggle="dropdown"><i class="bi bi-three-dots-vertical fs-6"></i></div>';
    codigo += '<ul class="dropdown-menu menuTabla text-center">';

    codigo +=
      '<li class="dropdown-item agregarPendTabla ' +
      config.color +
      '" data-id="' +
      datos[config.idUsuarioContenido] +
      '" data-bs-toggle="modal" data-bs-target="#modalAgregar"> Agregar </li>';
    codigo +=
      '<li class="dropdown-item info" data-id="' +
      datos[config.idUsuarioContenido] +
      '" data-bs-toggle="modal" data-bs-target="#modalPendiente"> Información </li>';
    codigo +=
      '<li class="dropdown-item eliminar" data-id="' +
      datos[config.idUsuarioContenido] +
      '" data-bs-toggle="modal" data-bs-target="#modalEliminar"> Eliminar </li>';

    return codigo;
  } else {
    let codigo =
      '<div class="position-relative"><div class="text-center"><span class="d-inline-block w-75">' +
      datos[config.nombre] +
      "</span></div>";
    ...
    codigo +=
      ...
      '" data-bs-toggle="modal" data-bs-target="#modalPendiente"> Información </li>';
    codigo +=
      '<li class="dropdown-item editarValoracion" data-id="' +
      datos[config.idUsuarioContenido] +
      '" data-bs-toggle="modal" data-bs-target="#modalEditar">Editar Valoración</li>';
    codigo +=
      '<li class="dropdown-item editarFecha" data-id="' +
      datos[config.idUsuarioContenido] +
      '" data-bs-toggle="modal" data-bs-target="#modalEditar">Editar Fecha' +
      config.pluralFecha +
      "</li>";
    if (config.editarEstado) {
      codigo +=
        '<li class="dropdown-item editarEstado" data-id="' +
        datos[config.idUsuarioContenido] +
        '" data-bs-toggle="modal" data-bs-target="#modalEditar">Editar Estado</li>';
    }
    if (config.editarPlataforma) {
      codigo +=
        '<li class="dropdown-item editarPlataforma" data-id="' +
        datos[config.idUsuarioContenido] +
        '" data-bs-toggle="modal" data-bs-target="#modalEditar">Editar Plataforma</li>';
    }
    codigo +=
      '<li class="dropdown-item eliminar" data-id="' +
      datos[config.idUsuarioContenido] +
      '" data-bs-toggle="modal" data-bs-target="#modalEliminar">Eliminar</li></ul></div></div>';
    ...
    if (datos[config.fav] == false) {
      if (datos[config.notas] == null) {
        html +=
          ...
          '" data-id="' +
          datos[config.idUsuarioContenido] +
          '" class="notas bi bi-journal fs-6" data-bs-toggle="modal" data-bs-target="#modalNotas"></i> ';
      } else {
        html +=
          ' <i id="notas' +
          datos[config.idUsuarioContenido] +
          '" data-id="' +
          datos[config.idUsuarioContenido] +
          '" class="notas bi bi-journal-text fs-6" data-bs-toggle="modal" data-bs-target="#modalNotas"></i> ';
      }
      html +=
        ' <i id="corazon' +
        datos[config.idUsuarioContenido] +
        '" data-id="' +
        datos[config.idUsuarioContenido] +
        '" class="fav bi bi-heart text-danger fs-6 ms-1"></i>';
      return html;
    } else {
      if (datos[config.notas] == null) {
        html +=
      ...
      html +=
        ...
        '" data-id="' +
        datos[config.idUsuarioContenido] +
        '" class="fav bi bi-heart-fill text-danger fs-6 ms-1"></i> ';
      return html;
    }
  }
} // Fin celdaIconos()
...
function tablaEstrellas(i) {
  $.ajax({
    ...
    if (respuesta.error) {
      ...
      window.tabla = crearTabla(
        respuesta,
        config.nombrePluralContenido +
          " valorad" +
          config.genero +
          "s con " +
          i +
...
function tablaFavoritas() {
  $.ajax({
    ...
    if (respuesta.error) {
      ...
      window.tabla = crearTabla(
        respuesta,
        config.nombrePluralContenido + " Favorit" + config.genero + "s",
      );
    } // Fin else
  }); // Fin ajax
} // Fin tablaFavoritas()
...
function crearTablaPendientes(lista) {
  ...
  if (lista.length == 0) {
    let vacio =
      '<div class="card"><div class="card-body text-center flex-column"> <i class="bi ' +
      config.icono +
      ' fs-2 d-block mb-2"></i> <strong class="text-black"> No hay registros</strong> </div></div>';
    return vacio;
  }
...
function resultadosApi(respuesta) {
  ...
  if (
    config.nombreContenido == "Serie" ||
    config.nombreContenido == "Temporada" ||
    config.nombreContenido == "Episodio"
  ) {
    poster = "posterSerie";
    nombre = "nombreSerie";
    ano = "anoSerie";
...
function mostrarDetalles(contenido, detalles, campoPoster = null) {
  ...
  const datosZona = detalles.filter((detalle) => detalle.zona === "datos");
  ...
  const abajoZona = detalles.filter((detalle) => detalle.zona === "abajo");
  ...
  const sinopsis = detalles.find((detalle) => detalle.zona === "sinopsis");
...
function mostrarSelectorTemporadas(respuesta) {
  ...
  for (const temporada of respuesta.temporadas) {
    ...
    const posterTemporada = temporada.poster_path
  ...
  for (const campo of form) {
    ...
    if (campo.type === "date") {
      ...
      if (campo.label == "Plataforma") {
        opciones = contenidoSeleccionado["plataformasJuego"]
          ? contenidoSeleccionado["plataformasJuego"]
              .split(",")
...
function mostrarFormPlataforma(contenido) {
  const opciones = contenido["plataformasJuego"]
    ? contenido["plataformasJuego"].split(",").map((opcion) => opcion.trim())
    : [];
  ...
  if (Number.isInteger(n)) {
    for (i = 1; i < n + 1; i++) {
      estrellas +=
        ...
        '" id="estrella' +
        i +
        '" class="estrella bi bi-star-fill"></i>';
    }
    for (i = 1; i < 6 - n; i++) {
      estrellas +=
        '<i data-valor="' +
        ...
        '" id="estrella' +
        (n + i) +
        '"class="estrella bi bi-star mx-1"></i>';
    }
  } else {
    for (i = 1; i < n; i++) {
      estrellas +=
        ...
        '" id="estrella' +
        i +
        '" class="estrella bi bi-star-fill mx-1"></i>';
    }
    ...
    estrellas +=
      ...
      '" id="estrella' +
      Math.ceil(n) +
      '"class="estrella bi bi-star-half mx-1"></i>';
...
function transformarRatingTabla(n) {
  ...
  if (Math.floor(n) == n) {
    for (let i = 0; i < n; i++) {
      estrellas +=
        '<i class="estrellaTabla bi bi-star-fill ' + config.color + '"></i>';
    }
  } else {
    for (let i = 0; i < n - 1; i++) {
      estrellas +=
    ...
    estrellas +=
      '<i class="estrellaTabla bi bi-star-half ' + config.color + '"></i>';
  }

  if (Math.floor(n) == n) {
    for (let i = 0; i < 5 - n; i++) {
      estrellas +=
        '<i class="estrellaTabla bi bi-star ' + config.color + '"></i>';
    }
  } else {
